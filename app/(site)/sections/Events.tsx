...
export default function Events({ data }: { data: any }) {
  ...
  return (
    <section className="container py-12 sm:py-16 relative" id="events">
      ...
      <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 mt-8 sm:mt-12">
        ...
        <ScrollStagger staggerDelay={0.1} className="space-y-1">
          ...
          <div className="relative pl-6 sm:pl-8 space-y-6 sm:space-y-8">
            {/* Vertical line */}
            <div className="absolute left-3 sm:left-4 top-0 bottom-0 w-0.5 bg-gradient-to-b from-pink-300 via-purple-300 to-blue-300"></div>
              ...
              return (
                <StaggerItem key={i}>
                  <div className="relative">
                    {/* Timeline dot */}
                    <div className={`absolute -left-5 sm:-left-6 w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-gradient-to-br ${colors[i] || 'from-pink-400 to-pink-600'} flex items-center justify-center shadow-lg ring-4 ring-white`}>
                      <span className="text-xs sm:text-sm">{icons[i] || '🎉'}</span>
                    </div>
                    
                    {/* Event card */}
                    <HoverScale scale={1.02}>
                      <div className="card ml-4 sm:ml-6 bg-white hover:shadow-xl transition-all duration-300">
                        <div className="flex flex-col sm:flex-row items-start gap-3 sm:gap-4">
                          {e.image && (
                            <div className="w-full sm:w-20 h-32 sm:h-20 rounded-lg overflow-hidden shadow-md flex-shrink-0">
                              <img src={e.image} alt={e.name} className="w-full h-full object-cover" />
                            </div>
                          )}
                          <div className="flex-1 w-full">
                            <h4 className="text-lg sm:text-xl font-bold text-gray-800 mb-1">{e.name}</h4>
                            <div className="flex items-center gap-2 text-sm text-gray-600 mb-2 flex-wrap">
                              <span>🕐</span>
                              <span className="leading-relaxed">{e.time}</span>
                            </div>
                            {e.description && (
                              <p className="text-sm text-gray-600 italic leading-relaxed">{e.description}</p>
                            )}
                          </div>
                        </div>
                      </div>
